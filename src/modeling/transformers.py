...
class ComputeDaysSinceStart(BaseEstimator, TransformerMixin, TransformerBase):
    ...
    def __init__(self, output_col: str, season_start_date: datetime.datetime):
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        print("ComputeDaysSinceStart")
        assert(len(X.columns.tolist()) == 1)
        self.input_col = X.columns.tolist()[0]
        X[self.output_col] = X[self.input_col].apply(lambda x: (x - self.season_start_date).days)
        return X[[self.output_col]]
